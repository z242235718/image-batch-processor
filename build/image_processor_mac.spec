...
#
# PyInstaller spec for Image Processor (macOS)
# Build: pyinstaller --workpath "build/pyinstaller-work" --distpath "build/dist" "build/image_processor_mac.spec"
#
# macOS 构建说明：
#   1. 需要 macOS 系统上运行 PyInstaller
#   2. 默认构建 当前架构 (arm64 或 x86_64)
#   3. 如需 universal binary：
#      - 在 Intel Mac 上构建 → x86_64
#      - 在 Apple Silicon Mac 上构建 → arm64
#      - 使用 lipo 合并：lipo -create -output ImageProcessor arm64_binary x86_64_binary
#
#   4. 如需构建 .app 包：在 spec 中设置 console=False
...
exe = EXE(
    ...
    upx_exclude=[],
    runtime_tmpdir=None,
    # console=True  → 命令行版本（终端运行，显示日志）
    # console=False → .app 包（双击运行，日志写入系统日志）
    console=True,
    disable_windowed_trailer=False,
    # macOS: 支持命令行参数（.app 拖放文件等）
    argv_emulation=False,
    # 自动匹配当前架构（arm64 或 x86_64）
    target_arch=None,
